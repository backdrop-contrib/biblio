...
/**
 * @file
 * Override of default node template.
 *
 * Differences:
 * - Never renders comments, submitted by, or links.
 * - In page view, adds the title as a field.
 *
 * Available variables:
...
<?php
  ...
  // Print the year.
  if ($field_biblio_year) {
    $needs_dot = TRUE;
    print ' (' . substr($field_biblio_year[0]['value'], 0, 4) . ')';
  }
  if ($needs_dot) {
    print ". ";
  }
  ...
  ?><a href="<?php print $node_url; ?>" rel="bookmark"><?php print $title; ?></a>. <?php

  // Print the secondary title, underlined, plus volume and pages.
  $needs_dot = FALSE;
  if ($field_biblio_secondary_title) {
    print '<span class="biblio-underlined">' . $field_biblio_secondary_title[0]['value'] . '</span>';
    $needs_dot = TRUE;
  }
  if ($field_biblio_volume) {
    print ' ' . $field_biblio_volume[0]['value'];
    $needs_dot = TRUE;
  }
  if ($field_biblio_pages) {
    $needs_dot = TRUE;
    print ', ' . $field_biblio_pages[0]['value'];
    $needs_dot = TRUE;
  }
  if ($needs_dot) {
    print '.';
